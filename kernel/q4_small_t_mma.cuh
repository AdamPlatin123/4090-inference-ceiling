...
// 动态共享内存大小（供 launch 侧计算并设置 MaxDynamicSharedMemorySize）
// kTileK 分档 + 缓冲策略: 小 T 用深 K 链单缓冲, 大 T 用浅 K 链双缓冲流水
template <class Geometry, int TileCols>
struct Q4SmallTSharedBytes {
    static constexpr int kWarps   = Q4DraftSmallTSchedule::kKWarps;
    static constexpr int kRows    = Q4DraftSmallTSchedule::kRowsPerCta;
    static constexpr int kTileK   = TileCols <= 16 ? 128 : 64;
    static constexpr bool kDouble = TileCols > 16;
    static constexpr int kGroupK  = kWarps * kTileK;
    static constexpr int kNt      = TileCols / 8;
    static constexpr std::size_t staging =
        (kDouble ? 2 : 1) * (static_cast<std::size_t>(kRows) * (kGroupK / 2) +
                             static_cast<std::size_t>(kWarps) * TileCols * kTileK *
                                 sizeof(__nv_bfloat16) +
                             static_cast<std::size_t>(kRows) * kWarps * (kTileK / 64) *
                                 sizeof(std::uint16_t));
    static constexpr std::size_t partial =
        static_cast<std::size_t>(kWarps) * kNt * 32 * 4 * sizeof(float);
    static constexpr std::size_t value = staging > partial ? staging : partial;
};
...
__launch_bounds__(256, 1) __global__
    ...
    constexpr int kTileK        = TileCols <= 16 ? 128 : 64;
    constexpr bool kDouble      = TileCols > 16;
    constexpr int kBufs         = kDouble ? 2 : 1;
    ...
    union SharedStorage {
        struct {
            std::uint8_t codes[kBufs][kRowsPerCta][kGroupK / 2];
            __nv_bfloat16 activations[kBufs][kWarps][kTileCols * kTileK];
            std::uint16_t scales[kBufs][kRowsPerCta][kWarps * (kTileK / 64)];
        } staging;

        float partial[kWarps * kNt * 32 * 4];
    };
    static_assert(sizeof(SharedStorage) == Q4SmallTSharedBytes<Geometry, TileCols>::value,
                  "dynamic shared memory size must match the kernel-side union");

    extern __shared__ __align__(16) char q4_small_t_dynamic_smem[];
    auto& shared       = *reinterpret_cast<SharedStorage*>(q4_small_t_dynamic_smem);
    ...
    const auto stage_x = [&](int group_k0, int buf) {
        ...
        for (int item = lane; item < kItemsPerSplit; item += 32) {
            ...
            auto* dst =
                &x_shared[buf][warp][col * kTileK + q4_small_t_swizzle_64(col, k8 * 8)];
    ...
    const auto stage_weight = [&](int group_k0, int buf) {
#pragma unroll
        ...
        for (int row = tid; row < kRowsPerCta; row += kWarps * 32) {
            const int weight_row = row_policy.weight_row(row0, row);
#pragma unroll
            for (int s = 0; s < kWarps * (kTileK / 64); s += 8) {
                cp_async<16>(&scale_shared[buf][row][s],
...
#pragma unroll
    for (int group_index = 0; group_index < kGroups; ++group_index) {
        ...
        const int buf      = kDouble ? (group_index & 1) : 0;

#pragma unroll
        for (int half = 0; half < kTileK / 64; ++half) {
            float group_acc[kNt][4] = {};

#pragma unroll
            for (int ks = 0; ks < 4; ++ks) {
                const int k_step = half * 4 + ks;
                const int byte_col = warp_koff / 2 + k_step * 8 + lid;
                const unsigned af0 = q4_small_t_bf16_pair(code_shared[buf][gid][byte_col]);
                const unsigned af1 = q4_small_t_bf16_pair(code_shared[buf][gid + 8][byte_col]);
                const unsigned af2 = q4_small_t_bf16_pair(code_shared[buf][gid][byte_col + 4]);
                const unsigned af3 = q4_small_t_bf16_pair(code_shared[buf][gid + 8][byte_col + 4]);
#pragma unroll
                for (int nt = 0; nt < kNt; ++nt) {
                    unsigned bf0, bf1;
                    const int br = nt * 8 + b_rin;
                    ldmatrix_x2(bf0, bf1,
                                smem_addr(&x_shared[buf][k_split][br * kTileK +
            ...
            const float top_scale = __half2float(
                __ushort_as_half(scale_shared[buf][gid][k_split * (kTileK / 64) + half]));
            const float bot_scale = __half2float(
                __ushort_as_half(scale_shared[buf][gid + 8][k_split * (kTileK / 64) + half]));
#pragma unroll
            for (int nt = 0; nt < kNt; ++nt) {
        ...
        if (group_index + 1 < kGroups) {
            if constexpr (kDouble) {
                cp_wait<0>();
                __syncthreads();
                if (group_index + 2 < kGroups) {
                    stage_weight((group_index + 2) * kGroupK, buf);
                    stage_x((group_index + 2) * kGroupK, buf);
                    cp_commit();
                }
            } else {
                __syncthreads();
                stage_weight(group_k0 + kGroupK, 0);
